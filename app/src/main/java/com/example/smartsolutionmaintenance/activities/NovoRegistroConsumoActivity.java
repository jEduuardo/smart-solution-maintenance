...
public class NovoRegistroConsumoActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        spinnerTipo.setAdapter(new ArrayAdapter<>(this,
                android.R.layout.simple_dropdown_item_1line, new String[]{"Água", "Energia"}));

        spinnerSetor.setAdapter(new ArrayAdapter<>(this,
                android.R.layout.simple_dropdown_item_1line,
                new String[]{"Administração","Sala de Aula","Cozinha","Banheiros",
    ...
    private void salvarConsumo() {
        ...
        try { valor = Double.parseDouble(valorStr); }
            ...
            Toast.makeText(this, "Valor inválido", Toast.LENGTH_SHORT).show(); return;
        }
        ...
        dados.put("observacao", edtObservacao.getText().toString().trim());
        dados.put("data", Timestamp.now());

        String leituraStr = edtLeitura.getText().toString().trim();
        if (!leituraStr.isEmpty()) {
            try { dados.put("leitura", Double.parseDouble(leituraStr)); }
